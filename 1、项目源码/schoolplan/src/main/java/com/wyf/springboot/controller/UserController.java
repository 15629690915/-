...
import com.wyf.springboot.config.AuthAccess;
...
import com.wyf.springboot.service.impl.EmailServiceImpl;
...
//        List<User> list = reader.readAll(User.class);
    ...
    public Result reset(@RequestBody UserPasswordDTO userPasswordDTO) {
        if (StrUtil.isBlank(userPasswordDTO.getEmail()) || StrUtil.isBlank(userPasswordDTO.getCode()) || StrUtil.isBlank(userPasswordDTO.getPassword()) || StrUtil.isBlank(userPasswordDTO.getNewPassword())) {
            throw new ServiceException("-1", "修改成功");
        }
        // 先查询 邮箱验证的表，看看之前有没有发送过  邮箱code，如果不存在，就重新获取
        QueryWrapper<Email> emailQueryWrapper = new QueryWrapper<>();
        emailQueryWrapper.eq("email", userPasswordDTO.getEmail());
        emailQueryWrapper.eq("code", userPasswordDTO.getCode());
        emailQueryWrapper.ge("time", new Date());  // 查询数据库没过期的code, where time >= new Date()
        Email one = emailService.getOne(emailQueryWrapper);
        if (one == null) {
